...
send_chunk_delay=${BUNIX_SEND_CHUNK_DELAY:-0.02}
...
export BUNIX_COLLECT_FAILURES BUNIX_FAILURE_DIR BUNIX_QEMU_LOG BUNIX_TEST_SIDECAR_LOG BUNIX_TEST_HARNESS BUNIX_TEST_RUNTIME_DIR BUNIX_SHELL_PARTS
	...
	if [ -n "$sidecar_ready" ]; then
		...
		while [ ! -e "$sidecar_ready" ]; do
			...
			if ! kill -0 "$sidecar_pid" 2>/dev/null; then
				fail_with_qemu_log "sidecar exited before readiness: $sidecar_ready" 120
			fi
			if [ "$i" -gt "$sidecar_ready_timeout" ]; then
				fail_with_qemu_log "sidecar readiness timed out: $sidecar_ready" 120
			fi
			sleep 1
		done
	else
		...
		if ! kill -0 "$sidecar_pid" 2>/dev/null; then
			fail_with_qemu_log "sidecar exited before qemu start" 120
		fi
	fi
}
