...
i=0
while ! grep -F "UNION_LOWER_RENAME_UPPER_OK" "$log" >/dev/null 2>&1; do
	i=$((i + 1))
	if [ "$i" -gt 45 ]; then
		echo "unionfs lower rename commands did not drain" >&2
...
i=0
while ! grep -F "PROC_SELF_CMDLINE_CALLER_OK" "$log" >/dev/null 2>&1; do
	i=$((i + 1))
	if [ "$i" -gt 45 ]; then
...
for expected in DEV_NULL_CHAR_OK DEV_ZERO_CHAR_OK DEV_CONSOLE_CHAR_OK; do
	...
	while ! grep -F "$expected" "$log" >/dev/null 2>&1; do
		i=$((i + 1))
		if [ "$i" -gt 45 ]; then
			echo "devfs character-device regression missing: $expected" >&2
			tail -n 180 "$log" >&2 || true
			exit 1
		fi
		sleep 1
