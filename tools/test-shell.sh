...
i=0
while ! grep -F "UNION_ROOT_LOWER_STILL_OK" "$log" >/dev/null 2>&1; do
	i=$((i + 1))
	if [ "$i" -gt 120 ]; then
		echo "unionfs root commands did not drain" >&2
...
if ! awk '{ sub(/\r$/, "") } /^READBIG_OK$/ { found = 1 } END { exit found ? 0 : 1 }' "$log"; then
	echo "large Linux read did not complete" >&2
	tail -n 220 "$log" >&2 || true
	exit 1
fi
