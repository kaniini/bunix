...
for expected in "hello.txt" "secret.txt" "musl-hello" "busybox" "cat" "stat" "File: /bin" "/bin" "File: /usr/share" "/usr/share" "File: /tmp" "/tmp" "File: /run" "/run" "File: /var/tmp" "/var/tmp" "File: '/usr/bin/env'" "/usr/bin/env" "nested" "SYMLINK_READLINK_OK" "SYMLINK_LS_OK" "lrwxrwxrwx"; do
	i=0
	while ! grep -F "$expected" "$log" >/dev/null 2>&1; do
		i=$((i + 1))
		if [ "$i" -gt 45 ]; then
