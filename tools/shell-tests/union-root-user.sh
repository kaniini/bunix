...
[ "$?" -eq 0 ] && echo UNION_ROOT_LOWER_OK
busybox mv /rename-lower.txt /rename-upper.txt
[ "$?" -eq 0 ] && echo UNION_LOWER_RENAME_CREATE_OK
busybox cat /rename-upper.txt | busybox grep "nested rootfs file"
[ "$?" -eq 0 ] && echo UNION_LOWER_RENAME_READ_OK
busybox test ! -e /rename-lower.txt
[ "$?" -eq 0 ] && echo UNION_LOWER_RENAME_OLD_GONE_OK
busybox test -e /.upper/rename-upper.txt
[ "$?" -eq 0 ] && echo UNION_LOWER_RENAME_UPPER_OK
EOF_UNION_RENAME
...
[ "$?" -eq 0 ] && echo UNION_ROOT_APPEND_CAT_OK
busybox cat /.upper/created.txt
[ "$?" -eq 0 ] && echo UNION_ROOT_UPPER_BACKING_OK
UNION_LONG_NAME=bunix-union-root-name-longer-than-sixteen.txt
echo UNION_ROOT_LONG_PAYLOAD > "/$UNION_LONG_NAME"
busybox ls / | busybox grep "$UNION_LONG_NAME"
[ "$?" -eq 0 ] && echo UNION_ROOT_LONG_READDIR_OK
busybox cat "/$UNION_LONG_NAME"
[ "$?" -eq 0 ] && echo UNION_ROOT_LONG_CAT_OK
...
busybox ls / | busybox grep "$UNION_NAME_MAX"
[ "$?" -eq 0 ] && echo UNION_NAME_MAX_READDIR_OK
busybox cat "/$UNION_NAME_MAX"
[ "$?" -eq 0 ] && echo UNION_NAME_MAX_CAT_OK
busybox ls / | busybox grep created.txt
[ "$?" -eq 0 ] && echo UNION_ROOT_READDIR_UPPER_OK
busybox ls / | busybox grep secret.txt
[ "$?" -eq 0 ] && echo UNION_ROOT_READDIR_LOWER_OK
busybox rm /created.txt
[ "$?" -eq 0 ] && echo UNION_ROOT_UNLINK_UPPER_OK
busybox test ! -e /created.txt
[ "$?" -eq 0 ] && echo UNION_ROOT_UNLINK_UPPER_GONE_OK
busybox cat /hello.txt
[ "$?" -eq 0 ] && echo UNION_ROOT_LOWER_STILL_OK
EOF_UNION_ROOT
