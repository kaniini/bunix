			    ...
			    const char *path)
{
	...
	const struct rootfs_entry *entry = nofollow ? rootfs_find(lower) :
					   rootfs_resolve(lower);
	if (entry == 0) {
		if (str_eq(lower, "/")) {
			stat_root_dir(reply);
		} else {
		       ...
		       const char *path)
{
	...
	if (service_path_call(tmpfs_service, BUNIX_VFS_OPEN_BUFFER, upper,
		...
		if (!task_can_access(message->words[3], entry,
