...
{
	const u64 service = message->words[0];

	if (service == 0 || message->cap == 0 ||
	    (message->cap_rights & BUNIX_RIGHT_SEND) == 0) {
		return -1;
	}

	for (unsigned int i = 0; i < MAX_NAMES; i++) {
		if (names[i].service == service) {
...
static const struct name_entry *resolve_name(u64 service)
{
	for (unsigned int i = 0; i < MAX_NAMES; i++) {
		if (names[i].service == service) {
			return &names[i];
	...
	for (;;) {
		...
		case BUNIX_NAMES_RESOLVE: {
			...
			const unsigned int rights =
				message.words[1] != 0 ?
				(unsigned int)message.words[1] : BUNIX_RIGHT_SEND;
