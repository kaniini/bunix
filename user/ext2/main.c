...
static u64 grant_vfs_caller_task(u64 sender, u64 task)
{
	...
	caller = (struct ext2_vfs_caller *)bunix_calloc(1, sizeof(*caller));
	if (caller == 0) {
		return (u64)-1;
	}
	caller->task = task;
...
static long inode_group_index(const struct ext2_mount *mount, u64 ino,
			      u64 *group_index)
{
	if (mount == 0 || group_index == 0 || ino == 0 ||
	    ino > mount->super.inodes_count ||
	    (ino - 1) / mount->super.inodes_per_group >=
		    mount->super.groups_count) {
		return -1;
...
static long adjust_group_used_dirs(struct ext2_mount *mount, u64 ino, int delta)
{
	u64 group_index;
	struct ext2_group *group;

	if (inode_group_index(mount, ino, &group_index) != 0) {
		return -1;
	}
	group = &mount->groups[group_index];
	if (delta > 0) {
		...
		if (group->used_dirs_count == 0) {
			return -1;
		}
		group->used_dirs_count--;
	}
	return write_group_count16(mount, group_index, 16,
				   group->used_dirs_count);
}
...
static long write_empty_dir_block(struct ext2_mount *mount, u64 block,
				  u64 ino, u64 parent_ino)
{
	const u64 dot_len = ext2_dirent_min_len(1);
	unsigned char *data;
	long result;

	if (mount == 0 || !block_range_valid(mount, block, 1) ||
	    mount->super.block_size <= dot_len) {
		return -1;
	}
	data = (unsigned char *)bunix_calloc(1, mount->super.block_size);
	if (data == 0) {
		return -1;
	}
	write_dirent(data, 0, ino, dot_len, 1, EXT2_FT_DIR, ".");
	write_dirent(data, dot_len, parent_ino, mount->super.block_size - dot_len,
		     2, EXT2_FT_DIR, "..");
	result = mount_write_bytes(mount, block_to_offset(mount, block), data,
				   mount->super.block_size);
	bunix_free(data);
	return result;
}
...
static long replace_dirent_inode(struct ext2_mount *mount,
				 const struct ext2_inode *directory,
				 u64 old_ino, const char *name, u64 new_ino,
				 u64 file_type)
{
	const u64 name_len = text_len(name);
	unsigned char *block;
	...
	for (u64 block_index = 0;
		...
		while (offset + 8 <= mount->super.block_size &&
			...
			if (entry_ino == old_ino &&
			    entry_name_len == name_len &&
			    text_eq_len(name, (const char *)block + offset + 8,
					entry_name_len)) {
				store_le32(block, offset, new_ino);
				block[offset + 7] = (unsigned char)file_type;
				if (write_mapped_block(mount, directory,
						       block_index,
						       block) != 0) {
...
static long update_dotdot(struct ext2_mount *mount,
			  const struct ext2_inode *directory, u64 parent_ino)
{
	unsigned char *block;
	long result = -1;

	if (mount == 0 || directory == 0 ||
	    ext2_inode_vfs_type(directory) != BUNIX_VFS_TYPE_DIRECTORY ||
	    directory->block[0] == 0 ||
	    !block_range_valid(mount, directory->block[0], 1)) {
		return -1;
	}
	block = (unsigned char *)bunix_calloc(1, mount->super.block_size);
	if (block == 0) {
	...
	if (read_mapped_block(mount, directory, 0, block) == 0) {
		u64 offset = 0;

		while (offset + 8 <= mount->super.block_size &&
		       offset < directory->size) {
			const u64 rec_len = load_le16(block, offset + 4);
			const u64 name_len = block[offset + 6];
			...
			if (name_len == 2 &&
			    text_eq_len("..", (const char *)block + offset + 8,
					name_len)) {
				store_le32(block, offset, parent_ino);
				result = write_mapped_block(mount, directory, 0,
							    block);
				break;
			}
			offset += rec_len;
...
static int directory_is_empty(const struct ext2_mount *mount,
			      const struct ext2_inode *directory)
{
	u64 cookie = 0;
	struct ext2_dirent entry;
	...
	return next_dirent(mount, directory, &cookie, &entry, 1) ==
	       BUNIX_VFS_ERR_NOENT;
}
...
static void reply_mkdir(struct bunix_msg *reply, const char *path, u64 mode)
{
	char parent_path[EXT2_MAX_PATH];
	char name[EXT2_NAME_MAX + 1];
	struct ext2_inode parent_inode;
	...
	u64 block = 0;
	u64 parent_links;
	const u64 sectors = root_mount.super.block_size / 512;

	if (!root_mount_ready || sectors == 0) {
		reply->words[0] = BUNIX_VFS_ERR_NOENT;
		return;
	}
	if (lookup_path(&root_mount, path, &existing_ino, &existing) == 0) {
	...
	if (parent_inode.links_count == 0xffff) {
		reply->words[0] = BUNIX_VFS_ERR_ACCESS;
		return;
	}
	parent_links = parent_inode.links_count;
	for (u64 i = 0; i < sizeof(inode.block) / sizeof(inode.block[0]); i++) {
		inode.block[i] = 0;
	}
	for (u64 i = 0; i < sizeof(inode.block_bytes); i++) {
	...
	inode.mode = EXT2_S_IFDIR | (mode & 07777);
	inode.uid = 0;
	inode.size = root_mount.super.block_size;
	inode.atime = 0;
	inode.ctime = 0;
	inode.mtime = 0;
	inode.gid = 0;
	...
	if (allocate_inode(&root_mount, &ino) != 0 ||
	    allocate_block(&root_mount, &block) != 0) {
		if (ino != 0) {
			(void)free_inode(&root_mount, ino);
		}
		reply->words[0] = (u64)-1;
	...
	inode.block[0] = block;
	if (write_empty_dir_block(&root_mount, block, ino, parent_ino) != 0 ||
	    write_inode_new(&root_mount, ino, &inode) != 0 ||
	    insert_dirent_existing_space(&root_mount, &parent_inode, ino, name,
					 EXT2_FT_DIR) != 0) {
		(void)free_block(&root_mount, block);
		(void)free_inode(&root_mount, ino);
		reply->words[0] = (u64)-1;
		return;
	}
	if (write_inode_links(&root_mount, parent_ino, &parent_inode,
			      parent_links + 1) != 0 ||
	    adjust_group_used_dirs(&root_mount, ino, 1) != 0) {
		(void)remove_dirent(&root_mount, &parent_inode, ino, name);
		(void)write_inode_links(&root_mount, parent_ino, &parent_inode,
					parent_links);
		(void)free_block(&root_mount, block);
		(void)clear_inode(&root_mount, ino);
		(void)free_inode(&root_mount, ino);
		reply->words[0] = (u64)-1;
		return;
	}
...
static int inode_has_direct_data_blocks(const struct ext2_inode *inode)
{
	if (inode == 0) {
		return 0;
	}
	for (u64 i = 0; i < EXT2_NDIR_BLOCKS; i++) {
		if (inode->block[i] != 0) {
			return 1;
		}
	}
...
static u64 ext2_file_type_for_inode(const struct ext2_inode *inode)
{
	switch (ext2_inode_vfs_type(inode)) {
	case BUNIX_VFS_TYPE_REGULAR:
		return EXT2_FT_REG_FILE;
	case BUNIX_VFS_TYPE_DIRECTORY:
		return EXT2_FT_DIR;
	case BUNIX_VFS_TYPE_SYMLINK:
		return EXT2_FT_SYMLINK;
	case BUNIX_VFS_TYPE_FIFO:
		return EXT2_FT_FIFO;
	case BUNIX_VFS_TYPE_CHARACTER:
...
static long release_unlinked_inode(struct ext2_mount *mount, u64 ino,
				   struct ext2_inode *inode)
{
	const u64 type = ext2_inode_vfs_type(inode);
	...
	if (inode->links_count > 1) {
		return write_inode_links(mount, ino, inode,
					 inode->links_count - 1);
	}
	if ((type == BUNIX_VFS_TYPE_REGULAR &&
	     free_direct_blocks_from(mount, ino, inode, 0) != 0) ||
	    clear_inode(mount, ino) != 0 ||
	    free_inode(mount, ino) != 0) {
		return -1;
	}
	return 0;
}

static long release_unlinked_directory(struct ext2_mount *mount, u64 ino,
				       struct ext2_inode *inode)
{
	if (mount == 0 || inode == 0 ||
	    ext2_inode_vfs_type(inode) != BUNIX_VFS_TYPE_DIRECTORY ||
	    inode_has_indirect_blocks(inode) ||
	    !inode_has_direct_data_blocks(inode) ||
	    !directory_is_empty(mount, inode)) {
		return -1;
	}
	if (free_direct_blocks_from(mount, ino, inode, 0) != 0 ||
	    adjust_group_used_dirs(mount, ino, -1) != 0 ||
	    clear_inode(mount, ino) != 0 ||
	    free_inode(mount, ino) != 0) {
		return -1;
	}
...
static void reply_unlink(struct bunix_msg *reply, const char *path)
{
	...
	if (release_unlinked_inode(&root_mount, ino, &inode) != 0) {
		reply->words[0] = (u64)-1;
		return;
	}
	reply->words[0] = 0;
...
static void reply_rename(struct bunix_msg *reply, const char *old_path,
			 const char *new_path, u64 flags)
{
	char old_parent_path[EXT2_MAX_PATH];
	char new_parent_path[EXT2_MAX_PATH];
	...
	u64 target_type;
	u64 file_type;
	u64 old_parent_links;
	u64 new_parent_links;
	int target_exists;
	int source_is_dir;
	int target_is_dir = 0;
	int moved_parent;
	...
	if (old_path == 0 || new_path == 0 ||
	    text_eq(old_path, "/") || text_eq(new_path, "/")) {
		reply->words[0] = BUNIX_VFS_ERR_NOENT;
		return;
	}
	if (text_eq(old_path, new_path)) {
		reply->words[0] = 0;
		return;
	}
	if (split_parent_name(old_path, old_parent_path, old_name) != 0 ||
	    split_parent_name(new_path, new_parent_path, new_name) != 0 ||
	    lookup_path(&root_mount, old_parent_path, &old_parent_ino,
			&old_parent_inode) != 0 ||
	    lookup_path(&root_mount, new_parent_path, &new_parent_ino,
			&new_parent_inode) != 0 ||
	    lookup_path(&root_mount, old_path, &source_ino,
			&source_inode) != 0) {
		reply->words[0] = BUNIX_VFS_ERR_NOENT;
		return;
	}
	source_type = ext2_inode_vfs_type(&source_inode);
	source_is_dir = source_type == BUNIX_VFS_TYPE_DIRECTORY;
	moved_parent = source_is_dir && old_parent_ino != new_parent_ino;
	if ((source_type != BUNIX_VFS_TYPE_REGULAR &&
	     source_type != BUNIX_VFS_TYPE_SYMLINK &&
	     source_type != BUNIX_VFS_TYPE_DIRECTORY) ||
	    (source_type == BUNIX_VFS_TYPE_REGULAR &&
	     inode_has_indirect_blocks(&source_inode)) ||
	    (source_type == BUNIX_VFS_TYPE_SYMLINK &&
	     source_inode.blocks != 0) ||
	    (source_type == BUNIX_VFS_TYPE_DIRECTORY &&
	     (inode_has_indirect_blocks(&source_inode) ||
	      !inode_has_direct_data_blocks(&source_inode) ||
	      path_has_prefix_child(new_path, old_path)))) {
		reply->words[0] = BUNIX_VFS_ERR_ACCESS;
		return;
	}
	file_type = ext2_file_type_for_inode(&source_inode);
	if (file_type == 0) {
		reply->words[0] = BUNIX_VFS_ERR_ACCESS;
		return;
	}
	target_exists = lookup_path(&root_mount, new_path, &target_ino,
				    &target_inode) == 0;
	if (target_exists && target_ino == source_ino) {
		reply->words[0] = 0;
		return;
	}
	if (target_exists && (flags & 1) != 0) {
		reply->words[0] = BUNIX_VFS_ERR_EXIST;
		return;
	}
	if (target_exists) {
		target_type = ext2_inode_vfs_type(&target_inode);
		target_is_dir = target_type == BUNIX_VFS_TYPE_DIRECTORY;
		...
		if ((target_type != BUNIX_VFS_TYPE_REGULAR &&
		     target_type != BUNIX_VFS_TYPE_SYMLINK &&
		     target_type != BUNIX_VFS_TYPE_DIRECTORY) ||
		    (target_type == BUNIX_VFS_TYPE_REGULAR &&
		     inode_has_indirect_blocks(&target_inode)) ||
		    (target_type == BUNIX_VFS_TYPE_SYMLINK &&
		     target_inode.blocks != 0) ||
		    (target_type == BUNIX_VFS_TYPE_DIRECTORY &&
		     (inode_has_indirect_blocks(&target_inode) ||
		      !inode_has_direct_data_blocks(&target_inode) ||
		      !directory_is_empty(&root_mount, &target_inode)))) {
			reply->words[0] = BUNIX_VFS_ERR_ACCESS;
			return;
		}
	}
	old_parent_links = old_parent_inode.links_count;
	new_parent_links = new_parent_inode.links_count;
	if (moved_parent &&
	    (old_parent_links == 0 ||
	     (!target_exists && new_parent_links == 0xffff))) {
		reply->words[0] = BUNIX_VFS_ERR_ACCESS;
		return;
	}
	if (moved_parent &&
	    update_dotdot(&root_mount, &source_inode, new_parent_ino) != 0) {
		reply->words[0] = (u64)-1;
		return;
	}
	if (target_exists) {
		if (replace_dirent_inode(&root_mount, &new_parent_inode,
					 target_ino, new_name, source_ino,
					 file_type) != 0) {
			if (moved_parent) {
				(void)update_dotdot(&root_mount, &source_inode,
						    old_parent_ino);
			}
			reply->words[0] = (u64)-1;
			return;
		}
	} else if (insert_dirent_existing_space(&root_mount, &new_parent_inode,
						source_ino, new_name,
						file_type) != 0) {
		if (moved_parent) {
			(void)update_dotdot(&root_mount, &source_inode,
	...
	if (remove_dirent(&root_mount, &old_parent_inode, source_ino,
			  old_name) != 0) {
		if (!target_exists) {
			(void)remove_dirent(&root_mount, &new_parent_inode,
					    source_ino, new_name);
		}
		if (moved_parent) {
			(void)update_dotdot(&root_mount, &source_inode,
	...
	if (moved_parent &&
	    (write_inode_links(&root_mount, old_parent_ino, &old_parent_inode,
			       old_parent_links - 1) != 0 ||
	     (!target_exists &&
	      write_inode_links(&root_mount, new_parent_ino, &new_parent_inode,
				new_parent_links + 1) != 0))) {
		reply->words[0] = (u64)-1;
		return;
	}
	if (target_exists &&
	    ((target_is_dir &&
	      release_unlinked_directory(&root_mount, target_ino,
					 &target_inode) != 0) ||
	     (!target_is_dir &&
	      release_unlinked_inode(&root_mount, target_ino,
				     &target_inode) != 0))) {
		reply->words[0] = (u64)-1;
		return;
	}
...
static void reply_rmdir(struct bunix_msg *reply, const char *path)
{
	char parent_path[EXT2_MAX_PATH];
	char name[EXT2_NAME_MAX + 1];
	struct ext2_inode parent_inode;
	...
	if (inode_has_indirect_blocks(&inode) ||
	    !inode_has_direct_data_blocks(&inode)) {
		reply->words[0] = BUNIX_VFS_ERR_ACCESS;
		return;
	}
	if (!directory_is_empty(&root_mount, &inode)) {
	...
	if (parent_inode.links_count > 0 &&
	    write_inode_links(&root_mount, parent_ino, &parent_inode,
			      parent_inode.links_count - 1) != 0) {
		reply->words[0] = (u64)-1;
		return;
	}
		...
		if (message.protocol == BUNIX_PROTO_VFS) {
			...
			case BUNIX_VFS_RENAME_BUFFER: {
				...
				if (read_two_path_buffer(&message, path,
					...
					reply_rename(&reply, path, new_path,
						     message.words[3] >> 32);
				}
				break;
			}
